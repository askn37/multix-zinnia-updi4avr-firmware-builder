...
HV制御を前提にしたFUSEを対象デバイスに書く場合、以下は常に忌避されるべきだ。

- tinyAVR なら PA0、それ以外なら PF7 が、GPIO出力に使われるべきではない。常に入力用か、RESET機能専用に限定する。
- `FUSE.SYSCFG1` の SUT 項目は、規定値の`0b111`以外にすべきではない。値が小さいほど外部制御の成功率は下がる。

### パケット順序番号でのリトライ制御

`JTAGICE mkII`パケット冒頭には順序番号が配されているが、これを正しく活用する実装に限り NVM書き込みのリトライが効率化される。

二線式シリアル通信は高速化（数Mbps以上）または長距離化（数千メートル以上、あるいは無線ブリッジ経由）すると相対的に信頼性が低下してパケットエラー率が増加する。この時どちらの方向でエラーが発生したかを区別するのに順序番号を使うことができる。端的にホスト側は、受信エラーを検出した際の同一順序番号を持つパケットを再びリクエストする。エンドノードは前回使用した順序番号を保存しておき、再び同一値が送られてきた時に次の判断を下すことができる。

- 順序番号が再送された：ホスト側へ向かう応答パケットが喪失した。
  - それが正常応答に対するものなら、如何なる作業も繰り返さず、前回と同じ正常応答を再び返す。
  - それが異常応答に対するものなら、その命令に従って処理を実行し、応答する。

この実装の要は、ホスト側が受信エラーのタイムアウトと、パケットロストを区別できるか否かだ。多くの簡易的な実装はこれを軽視しているので、同一パケット再送機能はなく、単にそれを捨てて新しい要求を再生成している。
...
> `0xff 0xff 0xff`が返される場合、それが配線ミスに起因するものでないならば、HV制御で回復できる可能性がある。しかし`0x00 0x00 0x00`が繰り返し返されるなら、その対象デバイスに対するHV制御もまた失敗する可能性が高い。
