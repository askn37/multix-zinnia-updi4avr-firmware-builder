...
[(en_US)](READEME_en.md) [(ja_JP)](READEME.md)
...
`AVRDUDE 7.2`以前はユーザーが独自のメモリ設定を追加して、`-U`でのメモリタイプ指定や、インタラクティブモードの`dump`、`read`、`write`コマンドで使うことができたが、`7.3`ではこれが使用できないか、あるいは大幅に応用が制限された。

### 一部のメモリ設定廃止

`AVRDUDE 7.2`で追加された`memory "sib"`対応は`7.3`で削除された。そもそも`SIB`（システムインフォーメーションブロック）は通常のIO空間には存在しない独立した情報なので、他のメモリタイプと同列で論じるのは理知的ではなかった。
